...
public class Ausgabe {
    ...
    public static void AusgabeImg(GpioPinDigitalOutput SI, GpioPinDigitalOutput SCK, GpioPinDigitalOutput RCK) {
        int Ausgabe[] = new int[190];
        for (int x = 1; x < 19; x++) {
            for (int y = 1; y < 11; y++) {

                if ((PiLed.image.getRGB(x-1,y-1)& 0xFF)!=0) {
                    if ((y > 1) & (y < 7)) {
                        Ausgabe[LED_Positon[y - 1][x - 1]] = 1;
                    } else {
                        Ausgabe[LED_Positon[y - 1][x - 1]] = 0;
                    }
                } else {
                    if ((y > 1) & (y < 7)) {
                        Ausgabe[LED_Positon[y - 1][x - 1]] = 0;
